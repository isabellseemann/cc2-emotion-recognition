...
class VideoDataModule(LightningDataModule):
    ...
    def _init_data(self):
        ...
        if (train_data_file.exists() and train_labels_file.exists()
            ...
            open_face_dataset = mmdatasdk.mmdataset(str(data_folder))
            video_tensors = open_face_dataset.get_tensors(seq_len=50, non_sequences=["All Labels"], direction=False,
            ...
            self.train_labels, self.train_features = cut_data(dominant_labels, video_tensors[0].get("OpenFace_2"))
            self.train_features = torch.tensor(np.array(self.train_features), dtype=torch.float32)
            self.val_labels = video_tensors[1].get("All Labels")
            self.val_features = torch.tensor(np.array(video_tensors[1].get("OpenFace_2")), dtype=torch.float32)
            self.test_labels = video_tensors[2].get("All Labels")
            self.test_features = torch.tensor(np.array(video_tensors[2].get("OpenFace_2")), dtype=torch.float32)
...
if __name__ == '__main__':
    data_module = VideoDataModule(batch_size=64, num_workers=0, pin_memory=False)
